...
import math as mt
import random
from Coded_Algorithms import Alg_conditions as AC
...
def sim_annealing(function, bounds, max_iter, max_eval, n=200, t_schedule=1, t_range=[1,0.05], step=1, seed=None):
    ...
    best_val = bounds[:, 0] + np.random.random(len(bounds)) * (bounds[:, 1] - bounds[:, 0])
    ...
    while j < max_iter:    # Outer loop iteration
        ...
        for _ in range(n):
            ...
            if config_eval < best_eval:
                ...
                if np.random.random() < exp(-diff / temp):
                    curr_eval, curr_val = config_eval, config_val
                else:
                    pass
...
def particle_swarm(function, bounds, max_iter, max_eval, n_particles = 200, parameter = [0.5,0.9,0.5], tol=1e-50, seed=None):
    ...
    for particle in range(n_particles):
        part_position[particle] = bounds[:, 0] + np.random.random(len(bounds)) * (bounds[:, 1] - bounds[:, 0])
...
def artificial_bee(function, bounds, max_iter, max_eval, n_bees=150, limit=12, tol=1e-50, seed=None):
    """
    ...
    food_source = np.array([bounds[:, 0] + np.random.random(len(bounds))* (bounds[:, 1] - bounds[:, 0]) for _ in range(n_food)])
    ...
    while i < max_iter:
        ...
        for k in scout_bees:
            if counter[k] > limit:
                food_source[k] = bounds[:,0] + np.random.random(len(bounds)) * (bounds[:,1]-bounds[:,0])
...
def diff_evolution(function, bounds, max_iter, max_eval, n_pop = 50, crossover=0.9, weight=0.3, tol=1e-50,seed = None):
    ...
    agent_coords = np.array([bounds[:, 0] + np.random.random(len(bounds))* (bounds[:, 1] - bounds[:, 0]) for _ in range(n_pop)])
...
def genetic_alg(function, bounds, max_iter, max_eval, n_pop = 50,  n_selection = 3, p_chld = 2, eta = 3, f_mut = 0.01, sigma = 0.8, tol=1e-50, seed = None):
    ...
    def mutation(x,f_mut,sigma):
        store = np.random.random(*x.shape) <= f_mut
    ...
    # Initialise data
    pop = np.array([bounds[:, 0] + np.random.random(len(bounds))* (bounds[:, 1] - bounds[:, 0]) for _ in range(n_pop)])
    ...
    while i < max_iter:
        ...
        for j in range(0,n_chld,2):
            ...
            # SBX operator
            beta = np.array([(2*mu)**(1/(eta+1)) if mu < 0.5 else (1/(2*(1-mu)))**(1/(eta+1)) for mu in np.random.random(dim)])
        ...
        size = int(0.05*n_pop)
        obj_counter += size
        new_pop = np.array([bounds[:, 0] + np.random.random(len(bounds))* (bounds[:, 1] - bounds[:, 0]) for _ in range(int(0.05*n_pop))])
...
def dh_simplex(function, point, bounds, max_iter, c_reflct=1, c_exp= 1.2, c_cont=.3, c_shrnk=.3, tol=5e-4, seed = None):
    ...
    #Parameter initialisation
    point_coords = np.array([np.clip(point + np.random.uniform(-0.2,0.2,size=dim)* 0.1*(bounds[:,1]-bounds[:,0]),bounds[:,0],bounds[:,1]) for _ in range(n_points)])
...
def cma_es(function,point, bounds, max_iter, sigma = 0.3):
    """
...
def space_reduction(function, bounds, max_iter, max_eval,n_pop = 100, n_glob = 5, f_reduction = 15, tol=1e-50, seed= None):
    ...
    #Parameter initialisation
    agent_coords = np.array([bounds[:, 0] + np.random.random(dim)* (bounds[:, 1] - bounds[:, 0]) for _ in range(n_pop)])
    agent_fit = np.apply_along_axis(function,1,agent_coords)
    best_eval = float('inf')
    ...
    while i < max_iter:
        ...
        for agent in range(n_pop):
            ...
            if (bounds[:,1] <= agent_coords[agent]).any() or (agent_coords[agent] <= bounds[:,0]).any():
                agent_coords[agent] = np.array(bounds[:, 0] + np.random.random(dim)* (bounds[:, 1] - bounds[:, 0]))
