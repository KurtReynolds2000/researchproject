...
from Coded_Algorithms.Algorithms import *
...
def mayfly_alg(function, bounds, max_iter, max_eval, n_fem=26, n_mal=26, f_mut = 0.01, tol=1e-80, seed=None):
    ...
    for i in range(mating.n_male):
        male_mayfly[i].Position = bounds[:,0] + np.random.random(dim)*(bounds[:,1]-bounds[:,0])
    ...
    for i in range(mating.n_female):
        female_mayfly[i].Position = bounds[:,0] + np.random.random(dim)*(bounds[:,1]-bounds[:,0])
...
def hybrid_differential(function , bounds, max_iter, max_eval, n_pop=100, repl = 0.85, f_mut = 0.01, tol=1e-80, seed = None):
    ...
    while i < max_iter:
        ...
        best_eval_last = np.copy(best_eval)
        ...
        if  criterium >= 1e-2:
            local_result, local_point,counter = dh_simplex(function,c[best_index],bounds,50,max_eval,seed=seed)
            obj_counter += counter
...
def improved_firefly_alg(function, bounds, max_iter, max_eval, pop_size=25, alphamax=0.001, betamin=2.24, gamma=0.0031, tol=1e-80,seed=None):
    
    """
    This is a function which follows the firefly algorithm
        - with a dynamic alpha and evolutionary traits
    """
    ...
    while k < max_iter:
        ...
        for i in range(pop_size):
            ...
            for j in range(pop_size):
                if intensity[i] > intensity[j]:
                    ...
                    steps = alpha * (np.random.uniform(-1,1,size=dim)) * delta
        ...
        alpha = alphamax - (alphamax - alphamin)*(np.max(intensity)-np.mean(intensity))/(np.max(intensity)-np.min(intensity))
